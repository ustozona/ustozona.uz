...
export default function ClassList({
  ...
  return (
    <div className="min-w-0 min-h-0 pr-4">
      <div className="h-full grid" data-tour="grades-class-selector">
            ...
            <div className="h-full overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              <div className="px-5 pt-1 pb-5 space-y-1">
                {CLASSES.map((cls) => {
                  ...
                  const hasData = !!classDataMap[cls.id] || cls.id === "no-class";
                  ...
                  if (isSelected) {
                    return (
                      <button
                        ...
                        data-state="closed"
                        className="w-full flex items-center text-left gap-3 p-4 border-2 rounded-xl cursor-pointer animate-spring-bounce data-[state=open]:ring-2 data-[state=open]:ring-inset data-[state=open]:ring-primary/40"
                        style={{
                  ...
                  return (
                    <button
                      ...
                      disabled={!hasData}
                      className="group w-full flex items-center text-left gap-2.5 px-3 py-2 border-2 border-transparent rounded-lg cursor-pointer transition-transform duration-200 ease-out hover:translate-x-1.5 data-[state=open]:ring-2 data-[state=open]:ring-inset data-[state=open]:ring-primary/40 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-x-0"
                    >
                      <div
          ...
          {selected && selectedData && hex && (
            <div className="group/stats border-t border-border px-5 py-5 space-y-4 shrink-0">
              ...
              >
                ...
                >
                  <p className="text-lg font-bold">{selectedData.students.length}</p>
                ...
                <div
                  ...
                  style={{ backgroundColor: `color-mix(in srgb, ${hex} 8.2%, transparent)` }}
                >
                  <p className="text-lg font-bold">{selectedData.assignments.length}</p>
                  <p className="text-[10px] text-muted-foreground">Topshiriqlar</p>
                </div>
              </div>

              <div className="space-y-1" data-state="closed" data-slot="tooltip-trigger">
