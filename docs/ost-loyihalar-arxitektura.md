...
⚠️ **Tuzatish (2026-08-16): R203 va R207 hujjatda "bajarildi" edi, kodda esa YOʻQ edi.** Yuqoridagi roʻyxat ikkita yozuvda kod bilan tarqalib ketgan — keyingi refaktorda tushib qolgan koʻrinadi. Ikkalasi ham shu kuni **haqiqatda** yozildi:
- **R203** — `Assignment.instructions` tipda, bazada, `grades-batch`/`grades-sync`/DAL da, `instructionsLabel`+`instructionsPlaceholder` esa oltita `messages/*.json` da **bor edi**, faqat maydon chizilmasdi. Endi sarlavhadan keyin `Textarea` boʻlib turadi; `isDraftDirty` ham uni koʻradi.
- **R207** — `buildScoreSuggestions()` `grades-data.ts` ga yozildi: taklif oʻqituvchining OʻZ jurnalidagi maxrajlar chastotasidan (teng boʻlsa kichigi oldinda), oxiriga standartlar (5·10·20·50·100) qoʻshiladi, roʻyxat 5 ta bilan cheklanadi. Chiplar maks. ball maydoni ostida; `setId` bilan qulflangan holatda **koʻrsatilmaydi** (bosilsa ishlamaydigan tugma chalgʻitardi).

Saboq: "Bajarildi" deb yozishdan oldin kodda tasdiqlang — hujjat kod bilan tarqalib ketsa, keyingi ish yoʻq narsaga tayanadi.
...
### Mazmun ↔ jurnal ustuni ajratilishi — Quizizz · Nearpod · Kahoot · Classroom · Canvas (R213–R220)

**R213 — ⭐⭐⭐ Baho ustuni TESTdan emas, YETKAZISHdan tugʻiladi.** Jahon tajribasi ikkiga boʻlinadi: **kontent-birinchi** (Quizizz, Kahoot, Nearpod, Wayground — kutubxona → yetkazish → hisobot → jurnalga eksport) va **jurnal-birinchi** (Classroom, Canvas — topshiriq asosiy birlik, mazmun unga *ilova*; jurnaldan ustun yaratib boʻlmaydi). Ustozona — **jurnal-birinchi** mahsulot: oʻqituvchi jurnalda yashaydi. Shu bois yuqori qavatda Classroom qoidasi, topshiriq **ichida** esa Quizizz qoidasi ishlaydi. Munosabat: bitta test → koʻp yetkazish → koʻp ustun. `publishSessionToGrades(sessionId, topicId)` allaqachon shunday — sxema toʻgʻri edi, UI esa "test = topshiriq" degan yolgʻon taassurot berardi (oʻqituvchi test tuzib saqlagach, Topshiriqlar roʻyxati boʻsh qolardi). ✅ Behroz `activity_sets.classId` filtrini olib tashlab (*"test istalgan sinfga berilsin"*) shu tamoyilni testlar uchun allaqachon oʻrnatgan.

**R214 — ⭐⭐⭐ Topshiriq oʻtkazishdan OLDIN yaratiladi. Dalil — qogʻoz rejimi.** Ustun `planned` holatida darhol paydo boʻladi, natija qanday yoʻl bilan kelishidan qatʼi nazar (jonli, mustaqil, OCR/OMR skaner, qoʻlda) oʻsha ustunga tushadi. ❌ Muqobil variant ("test oʻtkazilgandan keyin ustun tugʻilsin") **rad etildi**: `mode: paper` da sessiya umuman boʻlmaydi → ustun hech qachon yaratilmaydi → oʻqituvchi qogʻozdagi bahoni **qayerga yozishini bilmaydi**. Oʻzbekiston maktablarining katta qismida qurilma yetishmaydi, `paper`/`qrcards` aynan shuning uchun arxitekturaga kiritilgan — ularni oʻldiradigan model qabul qilinmaydi.

**R215 — `assignments.set_id` (nullable, `on delete set null`).** Topshiriqni mazmuniga bogʻlaydigan yagona halqa. `sourceSessionId` dan farqi: u **oʻtkazilgandan keyin** toʻladi (izlanuvchanlik), `set_id` esa **oldin** (reja). Oʻchirish semantikasi: topshiriq oʻchsa test **saqlanadi** (mazmun qayta ishlatiladi — Quizizz/Kahoot naqshi); test oʻchsa topshiriq va baholari qoladi, faqat halqa uziladi.

**R216 — ⚠️ Maks. ball tuzogʻi: test biriktirilgan topshiriqda u TAHRIRLANMAYDI.** Ssenariy: topshiriq yaratildi (maks. ball standart `100`), keyin testga 10 ta savol yozildi, qogʻozda oʻtkazildi. Oʻqituvchi "8" yozadi — u *8/10* demoqchi, tizim *8/100* deb oʻqiydi → **8%**. Jonli yoʻlda bu oʻz-oʻzidan tuzaladi (`publish.ts` maks. ballni savollar sonidan qayta hisoblaydi), **qogʻoz yoʻlida tuzalmaydi** — sessiya yoʻq, publish ishlamaydi. Qaror: `set_id` boʻlsa maks. ball savollar sonidan avtomatik va **faqat oʻqish**; oddiy (mazmunsiz) topshiriqda esa avvalgidek tahrirlanadi.

**R217 — ⭐⭐⭐ Fan sinfdan ham, oʻqituvchi profilidan ham meros olinmaydi.** Uch oʻqituvchi bilan sinaldi: *Bekzod aka* (faqat matematika) — ikkala manba ham ishlaydi; *Nodira opa* (7-A ga ham Algebra, ham Geometriya) — profil bitta fan saqlaydi ❌, sinf ham bitta fanga tegishli emas ❌; *Gulnora opa* (boshlangʻich 2-A, sakkizta fan) — ikkalasi ham sinadi ❌. Boshlangʻich sinf oʻqituvchilari — bozorimizning katta qismi. Qaror: **fan mazmunning oʻz xossasi** (dars/test oʻzi bilan olib yuradi); `classes.subject` va `teachers.subject` esa hokim emas, faqat **standart qiymat** beradi. Kelajak dalillari: oʻqituvchilar aro ulashish (mazmun oʻz pasporti bilan yurmasa qabul qiluvchida fansiz qoladi), standartlar bogʻlanishi (fan+daraja boʻyicha), maktab boshqaruvi kesimi, AI generatsiya.

**R218 — Fan `id` bilan saqlanadi, koʻrsatiladigan nom bilan emas.** Ilova 6 tilda: ruscha interfeysda saqlangan `"Математика"` va oʻzbekchada saqlangan `"Matematika"` bir xil fan ekanini tizim hech qachon bila olmasdi — qidiruv, filtr, ulashish buzilardi. ✅ `SUBJECT_CATALOG` (`standards-data.ts`): 28 fan `{ id, label }` shaklida, `subjectLabel()` / `subjectIdFromLabel()` / `normalizeSubject()` yordamchilari bilan. `id` **doimiy** — yozib boʻlingach oʻzgartirilmaydi. Eski `SUBJECTS` roʻyxati katalogdan hosil qilinadi (standart shablonlari nom boʻyicha moslashda davom etadi).

**R219 — ⭐⭐⭐ Dars sinfga emas, FAN+DARAJAga tegishli. `units` da ham shu kasal.** Bizda sinf — daraja idishi emas, **birga oʻsadigan toʻda**: `section` ("A") yildan yilga oʻzgarmaydi, `grade` esa faol yilga proyeksiya (`gradeByYear`). Oqibati: 5-sinf uchun yozilgan dars `5-A` ga bogʻlansa, keyingi yil oʻsha sinf **6-A** boʻladi — dars bolalar bilan birga yuqori sinfga koʻchib ketadi, yangi beshinchilar esa uni koʻrmaydi. Yaʼni bu noqulaylik emas, **maʼno xatosi**. Yarim yechim allaqachon bor (`classIds`, `unitByClass`, `scheduleByClass` — "bitta dars, koʻp sinf"), yetishmayotgani — yillar oʻrtasidagi oʻlcham. Strategiya tanlovi: ❌ **nusxa koʻchirish** (Classroom naqshi — 5 yildan keyin bir darsning 5 ta ajralgan nusxasi, biridagi yaxshilanish boshqasiga oʻtmaydi) vs ✅ **havola/kutubxona** (Canvas Blueprint, Planbook — bitta kanonik reja, sinf-yillar unga murojaat qiladi, sayqallash hammasiga tegadi). ⚠️ `units.classId` ham `notNull` — boʻlim ("Kasrlar") ham darajaga tegishli; darsni tuzatib boʻlimni tuzatmasak, dars koʻchadi-yu boʻlimi ortda qoladi.

**R220 — ⚠️ `quiz_sessions.mode` uchta oʻlchovni bitta enumga tiqadi.** Hozirgi qiymatlar `live | selfpaced | paper | qrcards | lecture` aslida uch mustaqil oʻqni aralashtiradi: **format** (viktorina · maʼruza/taqdimot · qogʻoz), **vaqt** (jonli · mustaqil), **javob usuli** (oʻz qurilmasi · QR karta · qogʻoz+OCR). Foydalanuvchi aniqlashtirdi: QR **ham** viktorinada, **ham** maʼruzada boʻladi va har oʻquvchida alohida — demak QR qoʻshni rejim emas, **javob usuli**. Hozirgi shaklda "maʼruza + QR" ni yozib boʻlmaydi; har yangi kombinatsiya enumga yangi qiymat qoʻshib portlaydi (`lecture_qr`, `quiz_selfpaced_qr`…). Hozir tegilmadi (ishlayotgan kodni buzmaslik uchun), lekin `mode_boundary` maydonining oʻzi ham rejimlarning toza ajralmasligini koʻrsatadi.

**Bajarildi:** R215 (sxema + tur + DAL + `drizzle/0030` — ⚠️ **hali qoʻllanmagan**, quyiga qarang), R218 (`SUBJECT_CATALOG` + yordamchilar). `SetBuilderOverlay` da: avtosaqlash (2 soniya, `saveSetDraftAction`) + yopishda ogohlantirish — ilgari 20 ta savol yozib `✕` bosilsa hammasi izsiz yoʻqolardi; kichraytirish (`_`, komponent unmount qilinmaydi); uchta chiqish yoʻlidan ikkitasi bir xil edi (`Chiqish` va `✕`) — matnli tugma olib tashlandi; 17 ta qattiq yozilgan matn `SetBuilder` namespace'iga koʻchdi (6 til).

**Qoldi:** R217/R219 (`Lesson.subject` + `Lesson.grade`, `units` shu bilan birga), R220 (`mode` oʻqlarga ajratilsin). Nomlash: jurnal ustuni = "Topshiriq", hozirgi *Topshiriqlar* sahifasi (mazmun uyi) boshqa nom oladi — **hal qilinmagan** (lekin 2026-08-16 dan keyin bosim kamaydi: sahifa endi barcha topshiriqni koʻrsatadi, faqat mazmunlisini emas).

✅ **R213/R214/R216 bajarildi (2026-08-16), sxema oʻzgarishisiz** — `set_id` ustuni allaqachon bor edi (R215), unga hech kim yozmasdi. Nima qilindi:

- **Mazmun — ilova, tur emas.** `kind` endi tanlanmaydi, `setId` dan HISOBLANADI. Mazmun boʻlimi `AssignmentEditorOverlay` da qoralamada ham, **tahrirda ham** chiziladi — ilgari u faqat qoralamada bor edi va yaratilgandan keyin test biriktirish yoʻli umuman yoʻq edi. ⚠️ Kuzatilgan alomat: mazmunsiz (`manual`) ustun tahrirga ochilganda «Test muharriri tez orada» degan **yolgʻon** boʻsh holat chiqardi — u test emas, toʻlaqonli baho ustuni edi.
- **"Test" tugmasi turni belgilamaydi, muharrirni ochadi.** `kind`/`setId` faqat toʻplam saqlanganda (avtosaqlash ham) yoziladi — «test deb belgilangan, orqasida hech nima yoʻq» holati tugʻilmaydi. Qoralama tashlansa toʻplam yetim qoladi va Topshiriqlar sahifasidagi «Biriktirilmagan testlar» roʻyxatida qayta ishlatiladi.
- **`publishSessionToGrades` endi uch holatni ajratadi:** (a) shu sessiyadan nashr qilingan qator, (b) **`set_id` orqali biriktirilgan va hali nashr koʻrmagan qator — toʻldiriladi**, (c) yoʻq boʻlsa yangisi. Ilgari faqat (a) va (c) bor edi: biriktirilgan test jonli oʻtkazilsa bitta test **ikkita** baho ustuni boʻlib chiqardi.
- **Mavjud toʻplamni biriktirish** (`AttachTestDialog`). Toʻplam muharrirdan **tashqarida** ham tugʻiladi — test bankidan olinganda, Baholash ish maydonida tuzilganda, qoralama tashlanganda. Ular «Biriktirilmagan testlar» boʻlib turardi, lekin ustunga ULASH yoʻli yoʻq edi: yagona tugma har doim yangi toʻplam yaratardi. ⚠️ Allaqachon bogʻlangan toʻplam roʻyxatda **qoladi** (faqat belgisi bilan) — R213 dagi "bitta test → koʻp yetkazish → koʻp ustun" munosabati bloklanmaydi (pre/post test naqshi).
- **R216 qulfi:** `setId` boʻlsa maks. ball `getSetMeta()` dan (elementlar soni, `publish.ts` bilan aynan bir formula) va faqat oʻqish.
- **Sync teshigi yopildi:** `setId` client push'ida yoʻq edi (server pull'ida esa bor edi) — halqa birinchi bootstrap'da yoʻqolardi. `assignmentUpsertSchema` + `toAssignmentUpsert` + `applyGradesBatch` ga qoʻshildi, egalik `ownedIds(activitySets)` bilan tekshiriladi.
- **Ikki eshik bitta shartnoma:** `manualCreate` bayrogʻi olib tashlandi. Ilgari Topshiriqlar sahifasidan «Yaratish» tur tanlanmaguncha **sababsiz oʻchiq** turardi (modal-ux qoidasini buzardi), chunki roʻyxat `kind` boʻyicha filtrlanardi va mazmunsiz topshiriq darhol gʻoyib boʻlardi. Filtr olib tashlandi.
- **Oraliq «Testlar (sinf)» ekrani oʻchirildi** (`TestWorkspaceOverlay` + `test/BaholashView` — ikkala fayl ham yoʻq). U sidebar'dan olib tashlangan `/dashboard/baholash` sahifasining qoldigʻi edi: oʻchirilish oʻrniga overlayga oʻralib, boshqa oqim ICHIGA qoʻyilgan. Muammolari: (a) uchinchi toʻliq-ekran qavati (`z-40` muharrir → `z-45` shu ekran → `z-48` savol muharriri), har birida oʻz `✕` i; (b) biriktirish oqimida savol muharriri darhol ochilgani uchun roʻyxat FAQAT muharrir yopilganda koʻrinardi — kutilmagan «qayerdaman?» ekrani; (c) nom va qalamcha AYNAN bir amalni bajarardi (R204 takrorlanishi); (d) `useTranslations` umuman yoʻq — oltita til buzuq; (e) oʻchirish native `confirm()` bilan (loyihada `AlertDialog` bor); (f) «Summativ» yorligʻi har doim shunday chiqardi, chunki `persist` da `purpose` qattiq yozilgan.
  **Endi:** savol muharriri va sessiya paneli topshiriq muharriridan **toʻgʻridan-toʻgʻri** ochiladi (`getSetAction` sessiya paneliga toʻliq qator beradi), sinf testlari roʻyxatining uyi esa Topshiriqlar sahifasi — u yerda «Biriktirilmagan testlar» kartalari kontekst menyusi bilan (Sessiya · Oʻchirish, `AlertDialog` bilan, oltita tilda). Qatlam 3 → 2. Jahon amaliyoti bilan mos: kutubxona — **manzil**, boshqa muharrir ichidagi bekat emas (Classroom: Drive **dialogi**; Canvas: savollar topshiriq ichida; Quizizz/Kahoot: kutubxona — bosh sahifa).
- 🐞 **`SetBuilderOverlay` da toʻplam IKKI marta yaratilardi.** `persist()` toʻplam id'sini React **holatidan** oʻqirdi; avtosaqlash ketayotganda «Saqlash» bosilsa (yoki sekin tarmoqda ikkinchi avtosaqlash tushsa) holat hali yangilanmagan boʻlib, ikkala soʻrov ham `setId: undefined` yuborardi. Alomat: roʻyxatda bir xil nomli ikkita test. Tuzatildi: id `useRef` da + yozuvlar navbati (`persistLock`).
- **Sarlavhadagi ikkita chip qayta oʻylandi** (2026-08-16, ikkinchi koʻrik).
  - ❌ **«Saqlandi» oʻchirildi.** U holat emas, `{!isDraft && <Badge/>}` — **konstanta** edi: hech qachon oʻzgarmasdi va sinxronizatsiya XATO berganda ham «Saqlandi» deb turaverardi. Jahon amaliyoti teskarisi: Notion hech nima koʻrsatmaydi, Google Docs/Figma qisqa koʻrsatib **soʻndiradi**; doimiy yonuvchi nishon bir haftada oʻqilmay qoladi. Oʻrniga `useSyncHealthStore` (yangi, qurilma-lokal) — `createServerSync` push xatosida `fail(scope)`, muvaffaqiyatda `ok(scope)` chaqiradi va muharrir **faqat muammo boʻlganda** «Saqlanmadi» chipini koʻrsatadi. Boʻlak nomi muhim: muharrir baholar store'iga yozadi, shuning uchun `useSyncFailing("grades")` — «bildirishnomalar sinxronlanmadi» uning sarlavhasida chiqmaydi.
  - **Holat chipi ikki turga ajratildi.** Kategoriya (Qoralama · Sanasiz · Rejalashtirilgan · **Bugun**) — soʻz; miqdor (baholash · tugallandi) — **halqa + `12/25`**. Sabab: «Baholanmoqda» alohida holat emas, `else` shoxi edi — sanasi bugun boʻlgan topshiriq yaratilgan zahoti, bironta baho boʻlmasa ham shu yorliqni olardi. Oʻqituvchining savoli «baholanyaptimi?» emas, «nechtasi qoldi?» (Classroom `12 Turned in`, Canvas `Needs Grading (7)`, Gradescope % halqasi, EMStudio `64 drafts` — hammasi RAQAM).
  - **Yangi «Bugun» holati** — `date === bugun` va hali baho yoʻq. Ilgari shart `eng_erta_sana > bugun` (qatʼiy katta) edi, shuning uchun bugungi nazorat ishi ertalab soat 8 da ham «Baholanmoqda» boʻlardi.
  - **Koʻp sinfda maxraj faqat sanasi kelgan sinflardan.** Ilgari `sort()[0]` — eng erta sana olinardi: 5-A dushanba, 5-B jumada boʻlsa, dushanbadan 5-B ning oʻquvchilari ham «baholanmagan» deb sanalardi. «Tugallandi» ham endi kelgusi sinf qolmagandagina beriladi.
  - `components/ui/progress-ring.tsx` — yangi primitiv. SVG matematikasi ilgari `AttendanceRing` (Statistika) ichida yolgʻiz edi; endi u shu primitivning ustidagi yupqa oʻram (svetofor rangi + markazdagi foiz), koʻrinishi piksel-aniq saqlandi (`radius` propi). Chipdagi halqa rangi — `currentColor`, yaʼni chip ohangidan meros.
- **Maks. ball oqibati aytiladi (R216 davomi).** Katakda **xom ball** turadi, foiz `ball ÷ maks. ball` dan hisoblanadi — maks. ballni 10 dan 100 ga oʻzgartirsangiz «8» oʻsha-oʻsha qoladi, lekin 80% → 8% boʻladi va jurnalga qarab buni **sezib boʻlmaydi**. Canvas/PowerSchool shu sabab `Save` tugmasi qoʻyadi; biz avtosaqlashni saqladik (global sessiya arxitekturasi — [[assignment-editor-global-session]]), lekin baholari bor ustunda oʻzgarish tugagach (`blur`, yoki chipda bir bosishda) **«{n} ta baho qayta hisoblandi»** + toast'da *Bekor qilish* chiqadi. Tugma faqat «qoʻllaymizmi?» deb soʻrardi — nechta baho oʻzgarishini aytmasdi.
- **Kichraytirish (`−`) tugmasi va suzuvchi yorliq oʻchirildi.** Ular Gmail «compose» naqshidan olingan edi, lekin Gmail'da xat uzun va bir necha qoralama stack boʻladi — bizda 6 ta maydon va ~15 soniya. Aslida `−` funksiya emas, **toʻliq-ekran idishga qoʻyilgan yamoq** edi: overlay butun ilovani yopgani uchun jurnalni koʻrishning boshqa yoʻli yoʻq edi. Uch nuqson: (a) `−` va ✕ dialogidagi «Qoralama sifatida saqlash» **aynan bir funksiyani** chaqirardi (`handleSaveAsDraft` → `minimize()`) — bitta amal ikki joyda; (b) 🐞 **parklangan qoralama jimgina yoʻqolardi** — `openDraft` shartsiz ustiga yozardi, yaʼni 5-A da qoralama qoldirib 5-B da «+» bosilsa ish izsiz ketardi, holbuki tugma «saqlandi» deb vaʼda qilardi; (c) qisqa forma uchun kechiktirish amalda kerak emas.
  **Yangi model — Classroom + Linear:** ✕ hech nima soʻramaydi (`park()`, qoralama qoladi); qoralama **Topshiriqlar roʻyxatida karta** boʻlib turadi (punktir ramka + «Qoralama» yorligʻi, Classroom `Draft` naqshi); «+» bosilganda tugallanmagan qoralama **tiklanadi**, ustiga yozilmaydi (Linear avto-tiklash) va bu toast bilan aytiladi; yoʻqotuvchi yagona amal — `⋯` → «Qoralamani oʻchirish». Natijada uchta tugma ikkitaga (`Yaratish` · `✕`) tushdi, bitta dialog va oltita tarjima kaliti oʻchdi. `minimized` → `parked` (persist v2 + migratsiya).
- ⚠️ **`assignments.group_id` ustuni hali YOʻQ** (R212 «1 ustun» deb yozgan edi). Koʻp-sinf guruhi faqat localStorage'da yashaydi va boshqa qurilmada yoʻqoladi — alohida bosqichda tuzatiladi.

✅ **`drizzle/0030` prodga qoʻllandi** (2026-08-09): `assignments.set_id` + FK yaratildi, drizzle jurnaliga yozildi (30 → 31 qator). Baza holati sogʻlom — baseline Supabase'ga koʻchishda **allaqachon** qilingan ekan (`0000`–`0029` yozilgan).

⚠️ Bir muddat bu yerda "baseline umuman qilinmagan, hech kim migratsiya qoʻya olmaydi" degan yozuv turgan edi — **u notoʻgʻri edi**. Sabab: tekshiruv soʻrovlari boshqa (boʻsh) Supabase loyihasida ishlatilgan. Saboq: `drizzle.__drizzle_migrations` yoʻq deb xulosa chiqarishdan oldin **qaysi loyihaga ulanganingizni** tasdiqlang — `select current_database(), count(*) from information_schema.tables where table_schema='public'` prod'da ~175 jadval qaytaradi, boʻsh loyihada 0.
...
28. ⚠️ **HAL QILINMAGAN ZIDDIYAT — ustun qachon tugʻiladi (R213/R214 ↔ "Tayyorlangan testlar").** Bir vaqtning oʻzida ikki qarama-qarshi model kodda yonma-yon turibdi:
    - **"Tayyorlangan testlar" (PR #16 atrofida, `assignments/page.tsx`):** test tuziladi va alohida roʻyxatda **kutadi**; jurnal ustuni faqat sessiya oʻtkazilib natija koʻchirilgandan keyin paydo boʻladi. UI matni buni ochiq aytadi: *"Bular hali jurnalda emas."*
    - **R213/R214:** ustun oʻtkazishdan **OLDIN** yaratiladi (`planned`), natija qaysi yoʻl bilan kelishidan qatʼi nazar oʻsha ustunga tushadi.

    Ziddiyatning oʻzagi — **qogʻoz rejimi**: `mode: paper` da sessiya umuman boʻlmaydi, demak birinchi modelda ustun **hech qachon** yaratilmaydi va oʻqituvchi qogʻozdagi bahoni yozadigan joy topmaydi. OCR/OMR skaner ham (PR #15 da qurila boshlagan) natijani **qayergadir** yozishi kerak. Ikkinchi model buni yechadi, lekin "hali oʻtkazilmagan test" uchun boʻsh ustun koʻrsatadi — bu esa jurnalni chalkashtirishi mumkin.

    Uchinchi variant ham bor: ustun **shakl tanlanganda** yaratilsin (jonli tanlansa — sessiyadan keyin, qogʻoz tanlansa — darhol). Hal qilinmagan; qaror qabul qilinmaguncha ikkala UI yonma-yon qolaveradi. `assignments.set_id` (R215) **har uchala** variantda kerak, shu bois u oldindan qoʻshildi.
